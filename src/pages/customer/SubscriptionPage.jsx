...
import { ArrowLeft, Package, ShoppingCart, Repeat, } from 'lucide-react';
    ...
    const handleFetchData = useCallback(async () => {
      if (!loading && !hasFetchedData) {
        try {
          ...
          await fetchSubscriptions(); // Fetch subscriptions for the logged-in user
          await fetchMenuItems({ merchantId: defaultMerchantId });
          setHasFetchedData(true);
        } catch (err) {
          console.error('Data fetch failed:', err);
          setHasFetchedData(true); // Prevent retries even on failure
        }
      }
                ...
                {subscriptions.length === 0 ? (
                  ...
                  subscriptions.map((sub) => (
                    <div key={sub.id} css={subscriptionCardStyles}>
                      <p css={cardTextStyles}><strong>Item:</strong> {sub.menuItem || 'Unknown Item'}</p>
                      <p css={cardTextStyles}><strong>Merchant:</strong> {sub.merchant || 'Unknown Merchant'}</p>
                      <p css={cardTextStyles}><strong>Schedule:</strong> {sub.schedule || 'N/A'}</p>
                      <p css={cardTextStyles}><strong>Start Date:</strong> {sub.start_date ? new Date(sub.start_date).toLocaleDateString() : 'N/A'}</p>
                      <p css={cardTextStyles}><strong>End Date:</strong> {sub.end_date ? new Date(sub.end_date).toLocaleDateString() : 'Ongoing'}</p>
                      <p css={cardTextStyles}><strong>Amount:</strong> ${sub.total_amount || '0.00'}</p>
                      <p css={cardTextStyles}><strong>Status:</strong> {sub.status || 'N/A'}</p>
                      <p css={cardTextStyles}><strong>Discount:</strong> {sub.discount || 'None'}</p>
                      {editMode === sub.id ? (
                        ...
                        <div>
                          <button
                            ...
                            onClick={() => {
                              setEditMode(sub.id);
                              setFormData({ schedule: sub.schedule || 'daily', end_date: sub.end_date || '' });
                            }}
                          >
                            Edit
                          </button>
